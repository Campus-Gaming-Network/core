...
export function environmentValidationIssues(environment: Environment): string[] {
  ...
  const siteURLValue = configuredValue(environment, "NEXT_PUBLIC_SITE_URL");
  if (strict && !siteURLValue) {
    issues.push("NEXT_PUBLIC_SITE_URL must be set");
  } else {
    const siteURL = parseHTTPOrigin(
      siteURLValue ?? localDefaults.NEXT_PUBLIC_SITE_URL
    );
    if (!siteURL) {
      issues.push("NEXT_PUBLIC_SITE_URL must be an absolute HTTP(S) origin");
    } else if (strict) {
      if (siteURL.protocol !== "https:") {
        issues.push("NEXT_PUBLIC_SITE_URL must use HTTPS");
      }
      if (isLocalHostname(siteURL.hostname)) {
        issues.push("NEXT_PUBLIC_SITE_URL must not use a local hostname");
      }
    }
  }
