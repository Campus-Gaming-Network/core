...
test("viewer relationship identifies self without serializing a second profile", async () => {
  ...
  assert.equal(
    JSON.stringify(result).includes("viewer-private@example.test"),
    false,
  );
});
...
test("viewer outages and response-contract failures are generic errors, never anonymous", async () => {
  ...
  assert.equal(
    JSON.stringify(outage).includes("sensitive_upstream_detail"),
    false,
  );
...
test("public profile presentation keeps metadata and outbound URLs safe", () => {
  ...
  assert.equal(
    JSON.stringify(metadata).includes("private@example.test"),
    false,
  );
  ...
  assert.equal(
    safeHTTPURL("https://example.test/player"),
