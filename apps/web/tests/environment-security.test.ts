...
const repositoryRoot = basename(currentDirectory) === "web"
...
test("allows deliberate local defaults without provider credentials", () => {
  ...
  assert.equal(validatedPublicOrigin({}), "http://localhost:3000");
});
...
const unsafeProductionSettings: Array<{
  ...
  },
  {
    ...
    values: { SITE_URL: "campusgamingnetwork.com" },
    expected: /SITE_URL must be an absolute HTTP\(S\) origin/
  },
  {
    name: "public site URL with path",
    values: { SITE_URL: "https://campusgamingnetwork.com/app" },
    expected: /SITE_URL must be an absolute HTTP\(S\) origin/
  },
  {
    name: "insecure public site URL",
    values: { SITE_URL: "http://campusgamingnetwork.com" },
    expected: /SITE_URL must use HTTPS/
  },
  {
    name: "local public site URL",
    values: { SITE_URL: "https://localhost:3000" },
    expected: /SITE_URL must not use a local hostname/
  },
  {
...
test("normalizes the validated site URL to a public origin", () => {
  assert.equal(
    validatedPublicOrigin({
...
test("Railway staging config uses the production image entry and health path", () => {
  const config = readFileSync(
    join(repositoryRoot, "railway/web.toml"),
    "utf8"
  );
  ...
  assert.doesNotMatch(config, /npm run dev|vite dev|apps\/web-start\/Dockerfile/);
});
