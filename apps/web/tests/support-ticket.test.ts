...
test("support operation works anonymously and strips its upstream response", async () => {
  ...
  assert.equal(
    JSON.stringify(result).includes(validInput.contact_email),
    false,
  );
...
test("support API and contract failures are safe and never serialize submitted content", async () => {
  const cases: Array<{ fetcher: Fetcher; message: string }> = [
    {
      fetcher: async () =>
        Response.json({ error: "support_ticket_failed" }, { status: 500 }),
      message: "We could not submit that support ticket. Please try again.",
    },
    {
      fetcher: async () =>
        Response.json({ id: "", private: validInput.message }),
      message: "Something went wrong. Please try again.",
    },
    {
      fetcher: async () => {
        throw new Error(`private: ${validInput.message}`);
      },
      message: "Something went wrong. Please try again.",
    },
  ];
  ...
  await Promise.all(
    cases.map(async ({ fetcher, message }) => {
...
test("native support notices accept only the first known enum", () => {
  ...
  assert.deepEqual(
    validateSupportSearch({ support: "private-upstream-code" }),
    {},
  );
});
