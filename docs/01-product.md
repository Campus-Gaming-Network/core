...
- Browse/filter events by game (and other filters) — no near-you yet
...
- Optional assignment to a school club (Varsity, JV, etc.) — once clubs exist
...
- Optional capacity; when full, no more registrations (no waitlist yet)
...
- Impersonate (“mimic”) users to see their permissions (later)
- Feature flags — **not scheduled** (later)
