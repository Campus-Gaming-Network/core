...
| Application errors | Domain packages return transport-neutral typed errors with stable machine codes for validation, unprocessable references, missing resources, conflicts, authentication, and authorization. `internal/httpapi/error_mapper.go` is the only application-error-to-HTTP policy. Error wrapping must preserve `errors.Is`/`errors.As`; response bodies expose only the stable code, never the diagnostic cause or message. |
| Go package/test layout | Organize large domain packages by current capability instead of adding framework layers. The event package separates types, validation, model helpers, commands, queries, persistence, scanning, and recurrence while retaining one package boundary. HTTP handler tests live in route-family files with shared fakes and fixtures. TanStack Start server functions and operations remain feature-local. |
...
| Area | Decision |
|------|----------|
| Recurrence | Creation supports weekly, biweekly, and monthly schedules through an inclusive end date interpreted in the event's IANA timezone, no more than one year after the first occurrence. Occurrence starts retain the original local wall-clock time. A nonexistent spring-forward start moves forward by the transition gap, while a repeated fall-back start uses the earlier instant. Elapsed duration is preserved even if the local end time changes. Monthly occurrences clamp independently from the original day anchor, returning to that day when a later month supports it. Each occurrence is a normal event row with its own slug, RSVPs, and cancellation lifecycle. |
...
| Event time entry | Create/edit forms accept local wall-clock values through native `datetime-local` controls and use a curated US timezone selector defaulted from the profile. The web server function converts valid local values to UTC instants before calling Go. Nonexistent spring-forward times and ambiguous fall-back times are rejected rather than guessed for the initial event; derived recurrence starts follow the recurrence DST policy above. |
...
| CI | Pull requests must run backend tests, frontend type/lint/test checks, the fast fake-API browser suite, and the focused real-stack browser suite. Do not lower existing coverage or remove regression tests to make a change pass. |
