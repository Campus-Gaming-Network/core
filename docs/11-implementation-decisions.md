...
| Deployment safety mode | `DEPLOYMENT_ENV` is `local`, `staging`, or `production`. Direct development defaults to `local`, production container images default to `production`, and Compose explicitly selects `local`. Staging and production use the same fail-closed startup checks in both web and API; `NODE_ENV` is not a trust signal because Next.js also sets it for local production builds. Local mode alone permits localhost URLs, insecure cookies, and disabled external email/edge credentials. |
