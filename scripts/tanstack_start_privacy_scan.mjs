...
const appRoot = path.join(repositoryRoot, "apps", "web-start");
...
assert.ok(
  ...
  "TanStack Start public output is missing; build apps/web-start before scanning"
);
...
if (findings.length > 0) {
  process.stderr.write("TanStack Start privacy/static scan failed:\n");
  for (const finding of findings) process.stderr.write(`- ${finding}\n`);
  process.exitCode = 1;
} else {
  process.stdout.write(
    `TanStack Start privacy/static scan passed: ${clientFiles.length} public files contain no server markers and application source contains no Next.js boundary.\n`
  );
}
